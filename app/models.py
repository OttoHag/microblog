...
# Følger-tabell for mange-til-mange-relasjon
followers = sa.Table(
    'followers',
    db.metadata,
...
class User(UserMixin, db.Model):
    ...
    def followed_posts(self):
        followed = sa.select(Post).options(joinedload(Post.author)).join(
            followers, followers.c.followed_id == Post.user_id
        ).where(followers.c.follower_id == self.id)

        own = sa.select(Post).where(Post.user_id == self.id)

        union_query = followed.union_all(own).order_by(Post.timestamp.desc())

        return db.session.execute(union_query).unique().scalars()
